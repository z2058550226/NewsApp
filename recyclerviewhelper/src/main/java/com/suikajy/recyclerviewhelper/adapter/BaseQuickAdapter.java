package com.suikajy.recyclerviewhelper.adapter;
...
import com.suikajy.recyclerviewhelper.R;
import com.suikajy.recyclerviewhelper.helper.ItemTouchHelperAdapter;
import com.suikajy.recyclerviewhelper.helper.OnStartDragListener;
import com.suikajy.recyclerviewhelper.helper.SimpleItemTouchHelperCallback;
import com.suikajy.recyclerviewhelper.listener.OnItemMoveListener;
import com.suikajy.recyclerviewhelper.listener.OnRecyclerViewItemClickListener;
 ...
 * RecyclerView适配器基类
 * 这个类将正常的RecyclerView分解为头部, 数据部, 尾部, 加载中视图, 空视图, full span视图
 * 头部尾部等的类型不需要子类去管理, 子类如果有多种类型的数据, 就使用这个类的子类BaseMultiItemQuickAdapter
 * <p>
 * 这个类主要功能 :
 * 1.省去常用布局分类的代码
 * 2.方便添加删除数据项,监听接口
 * 3.实现了拖拽功能 - ItemTouchHelper
 * 4.增加了加载中判断, 避免重复加载的发生, 但加载完毕需要手动调用loadComplete来通知Adapter
 * 5.自带上拉加载的界面, 滑动到最底部时会自动显示. 使用条件是添加一个OnRequestDataListener
 * 6.当使用GridLayoutManager的时候, 可以很方便的控制item的span跨度
 */
